		...
		<div id="contentwrapper">
			...
			<div id="maintitle">
                ...
                <div id="standardtoolbox">
					...
					$subjectLinkHasBeenGenerated = false;
					$historyOrProtectLinkHasBeenGenerated = false;
					$moreToolsLinkHasBeenGenerated = false;
					
					foreach ( $this->data['namespace_urls'] as $link ) {
						if ( !$subjectLinkHasBeenGenerated ) { //Only generating the subject link, not the discussion link (discussion link is handled by next foreach). If it's generated by this foreach the appropriate title text, etc. isn't provided, but if it's generated below everything is provided.
							echo '<a class="' . $link['class'] . '" ' .
										'id="' . $link['id'] . '" ' .
										'href="' . htmlspecialchars( $link['href'] ) . '">' .
										htmlspecialchars( $link['text'] ) . '</a>';
							$subjectLinkHasBeenGenerated = true;
						}
                    }
					
					$toolbox = $this->getToolbox();
					
        			if ( sizeof( $this->data['content_actions'] ) > 1 ) { //if there are tools like "edit," etc. in addition to non-page-specific ones like "special pages" //also check if logged in
						foreach ( $this->data['content_actions'] as $key => $action ) {
							if ( !$historyOrProtectLinkHasBeenGenerated ) {
								echo $this->makeLink( $key, $action );
								if ( $key == "history" || $key == "protect" ) {
									$historyOrProtectLinkHasBeenGenerated = true;
