...
window.Refreshed = {
	...
	showHideOverflowingDropdowns: function() {
		$( '.page-item-has-children' ).each(function( ) {
			if ( $( this ).offset().top > $( '#header-inner' ).height() + $( '#header-inner' ).offset().top ) { //if the .page-item is beneath the bottom of the header (and so it's cut off by overflow:hidden)
...
$( document ).ready( function() {
	...
	$( document ).on( 'tap', function( e ) {
		...
		*/
		...
		if ( Refreshed.headerSearchIsOpen && !$( e.target ).closest( '#header-inner .search' ).length && !$( e.target ).closest( '.header-suggestions' ).length ) { // we do this check instead of checking if the user pressed #fade-overlay because #fade-overlay can disappear if you resize, and then if you click off afterward you still want to hide the menu, etc. even if #fade-overlay is no longer visible
	 ...
	 * there is one for the #header-inner search bar and one for the #bodyContentsearch bar.
	 * We only want to target the one for the #header-inner search bar.
	 */
